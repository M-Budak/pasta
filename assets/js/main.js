...
document.addEventListener("DOMContentLoaded", function() {
  ...
  .then(rep => {
      ...
      for (let i = 0; i < items.length; i++) {
          ...
          if (nameElement) {
              nameElement.innerHTML = data.table.rows[row].c[1] && data.table.rows[row].c[1].v ? data.table.rows[row].c[1].v : '';
          }

          // Menü içeriği
          let descriptionElement = document.getElementById(item + '_description');
          if (descriptionElement) {
              descriptionElement.innerHTML = data.table.rows[row].c[2] && data.table.rows[row].c[2].v ? data.table.rows[row].c[2].v : '';
          }

          // Menü fiyatı
          let priceElement = document.getElementById(item + '_price');
          if (priceElement) {
              priceElement.innerHTML = (data.table.rows[row].c[3] && data.table.rows[row].c[3].v ? data.table.rows[row].c[3].v : '') + " TL";
          }
      }
  })
