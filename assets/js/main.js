  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    .then(rep => {
        ...
        for (let i = 0; i < items.length; i++) {
            let item = items[i];
            let row = i; // Satır indeksini belirle
  
            // Menü adı
            let nameElement = document.getElementById(item + '_name');
            if (nameElement) {
                nameElement.innerHTML = data.table.rows[row].c[0] && data.table.rows[row].c[0].v ? data.table.rows[row].c[0].v : 'Yok';
            }
  
            // Menü içeriği
            let descriptionElement = document.getElementById(item + '_description');
            if (descriptionElement) {
                descriptionElement.innerHTML = data.table.rows[row].c[1] && data.table.rows[row].c[1].v ? data.table.rows[row].c[1].v : 'Açıklama mevcut değil';
            }
  
            // Menü fiyatı
            let priceElement = document.getElementById(item + '_price');
            if (priceElement) {
                priceElement.innerHTML = (data.table.rows[row].c[3] && data.table.rows[row].c[3].v ? data.table.rows[row].c[3].v : 'Fiyat bilinmiyor') + " TL";
            }
        }
    })
